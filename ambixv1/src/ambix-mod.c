 ...
 * <r68karimi@gmail.com>
 * @see https://github.com/miguelmarques1904/ambix for a full description of the
 * module.
 */
...
#define PAGE_WALK_THRESHOLD 5001752
...
static int find_target_process(
    pid_t pid) { // to find the task struct by process_name or pid
  if (n_pids >= MAX_PIDS) {
...
static int refresh_pids(void) {
  ...
  for (i = 0; i < n_pids; i++) {
    if ((task_items[i] == NULL) || (find_get_pid(task_items[i]->pid) == NULL)) {
      update_pid_list(i); // TODO giga cursed, rewrite
      i--;
    }
  }
...
static int pte_callback_mem(pte_t *ptep, unsigned long addr, unsigned long next,
  ...
  if (n_found == n_to_find || pages_walked > PAGE_WALK_THRESHOLD) {
    last_addr_dram = addr;
    return 1;
  }
...
static int pte_callback_nvram_clear(pte_t *ptep, unsigned long addr,
  ...
  if (pages_walked > PAGE_WALK_THRESHOLD) {
    return 1;
  }
...
static int do_page_walk(struct mm_walk_ops mem_walk_ops, int last_pid,
  ...
  if (mm != NULL) {
    mmap_read_lock(mm);
    g_walk_page_range(mm, last_addr, MAX_ADDRESS, &mem_walk_ops, NULL);
    mmap_read_unlock(mm);
  }
  ...
  if (mm != NULL) {
    mmap_read_lock(mm);
    g_walk_page_range(mm, 0, last_addr + 1, &mem_walk_ops, NULL);
    mmap_read_unlock(mm);
  }
...
static int mem_walk(int n, int mode) {
  ...
  if (dram_walk) {
    ...
    last_pid_nvram =
        do_page_walk(mem_walk_ops, last_pid_nvram, last_addr_nvram);
  }
...
static int clear_walk(int mode) {
  ...
  for (i = 0; i < n_pids; i++) {
    ...
    curr_pid = task_items[i]->pid;
    g_walk_page_range(
        mm, 0, MAX_ADDRESS, &mem_walk_ops,
        NULL); // TODO usar last_addr para "segmentar" a chamada a isto
               //  Suspeitamos que seja isto que causa o stack trace
...
static int switch_walk(int n) {
  ...
  n_found++;
  n_to_find =
      n_found + dram_to_find; // try to find the same amount of dram addrs
  n_backup = 0;
  ...
  if (dram_found == nvram_found) {
    ...
    if (n_backup < remaining) {
      ...
      nvram_found =
          dram_found + n_backup; // update nvram_found and discard other entries
      int new_dram_start = nvram_found + 1;
      found_addrs[nvram_found].pid_retval =
          0; // fill separator after nvram pages

      int i;
      for (i = 0; i < dram_found; i++) {
        found_addrs[new_dram_start + i].addr =
            found_addrs[old_dram_start + i].addr;
        found_addrs[new_dram_start + i].pid_retval =
            found_addrs[old_dram_start + i].pid_retval;
      }
      to_add = n_backup;
...
static void print_mode(int mode) {
  ...
  switch (mode) {
  case DRAM_MODE:
    printk(KERN_CONT "DRAM_NODE\n");
    break;
  case NVRAM_MODE:
    printk(KERN_CONT "NVRAM_MODE\n");
    break;
  case NVRAM_WRITE_MODE:
    printk(KERN_CONT "NVRAM_WRITE_MODE\n");
    break;
  case NVRAM_INTENSIVE_MODE:
    printk(KERN_CONT "NVRAM_INTENSIVE_MODE\n");
    break;
  case NVRAM_CLEAR:
    printk(KERN_CONT "NVRAM_CLEAR\n");
    break;
  case SWITCH_MODE:
    printk(KERN_CONT "SWITCH_MODE\n");
    break;
  }
}
...
static void process_req(req_t *req) {
  ...
  if (req != NULL) {
    switch (req->op_code) {
    case FIND_OP:
      printk(KERN_CONT "Refreshing PIDs\n");
      refresh_pids();
      if (n_pids > 0) {
        int n = 0;
        print_mode(req->mode);
        ...
        case NVRAM_INTENSIVE_MODE:
          ...
          printk(KERN_CONT "Calling mem_walk\n");
          ret = mem_walk(n, req->mode);
          printk(KERN_CONT "Walked %llu pages\n", pages_walked);
          break;
        case NVRAM_CLEAR:
          clear_walk(req->mode);
          printk(KERN_CONT "Walked %llu pages\n", pages_walked);
...
static void placement_nl_process_msg(struct sk_buff *skb) {
  ...
  int required_packets =
      (n_found / MAX_N_PER_PACKET) + (last_packet_remainder != 0);
  ...
  for (i = 0; i < required_packets - 1; i++) { // process all but last packet
    nlmh_array[i] = nlmsg_put(
        skb_out, 0, 0, 0, MAX_N_PER_PACKET * sizeof(addr_info_t), NLM_F_MULTI);
    memset(NLMSG_DATA(nlmh_array[i]), 0, MAX_PAYLOAD);
    memcpy(NLMSG_DATA(nlmh_array[i]), found_addrs + i * MAX_N_PER_PACKET,
           MAX_PAYLOAD);
  }
