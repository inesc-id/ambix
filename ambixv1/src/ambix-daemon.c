...
int do_migration(int mode, int n_found) {
  ...
  for (n_migrated = 0, i = 0; n_migrated < n_processed; n_migrated += i) {
    ...
    for (i = 1; (candidates[n_migrated + i].pid_retval == curr_pid) &&
                (n_migrated + i < n_processed);
         i++)
      ;
...
int do_switch(int n_found) {
  ...
  while ((((dram_migrated + dram_e) < n_found) ||
          ((nvram_migrated + nvram_e) < n_found)) &&
         (dram_free || nvram_free)) {
    // DRAM -> NVRAM
    ...
    for (int i = 0; (i < n_nvram_nodes) && (dram_processed < n_found); i++) {
      ...
      for (; (j < n_avail_pages) && (j + dram_processed < n_found); j++) {
        addr_dram[dram_processed + j] =
            (void *)candidates[n_found + 1 + j].addr;
    ...
    if (old_n_processed < dram_processed) {
      ...
      for (n_migrated = 0, i = 0; n_migrated < dram_processed;
           n_migrated += i) {
        int curr_pid;
        curr_pid = candidates[n_found + 1 + n_migrated].pid_retval;

        for (i = 1; (candidates[n_found + 1 + n_migrated + i].pid_retval ==
                     curr_pid) &&
                    (n_migrated + i < dram_processed);
             i++)
          ;
    ...
    for (int i = 0; (i < n_dram_nodes) && (nvram_processed < n_found); i++) {
      ...
      for (; (j < n_avail_pages) && (j + nvram_processed < n_found); j++) {
        addr_nvram[nvram_processed + j] =
            (void *)candidates[nvram_processed + j].addr;
    ...
    if (old_n_processed < nvram_processed) {
      ...
      for (n_migrated = 0, i = 0; n_migrated < nvram_processed;
           n_migrated += i) {
        int curr_pid;
        curr_pid = candidates[n_migrated].pid_retval;

        for (i = 1; (candidates[n_migrated + i].pid_retval == curr_pid) &&
                    (n_migrated + i < nvram_processed);
             i++)
          ;
...
void *memcheck_placement(void *args) {
  ...
  while (!exit_sig) {
    ...
    if (n_migrated > 0) {
      sleep_interval *=
          2; // give time for bw to settle given the migrated pages
      if (switch_migrated > 0) {
  ...
  while (!exit_sig) {
    ...
    if (sel == -1) {
      ...
      if (rd < 0) {
        ...
        fprintf(stderr, "Unexpected amount of bytes read from accepted UD "
                        "socket connection.\n");
      }
    }
  }
