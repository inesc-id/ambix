  ...
  for (imc_i = 0; imc_i < IMCs_size; ++imc_i) {
    ...
    for (cpu_i = 0; cpu_i < CPUs_size; ++cpu_i) {
      for (ctr_i = 0; ctr_i < COUNTERs_size; ++ctr_i) {
        ...
        evt = perf_event_create_kernel_counter(&event_attr, cpu, NULL, NULL,
                                               NULL);

        pr_debug("%lu: cpu:%d; imc:%d ev:%llx; name:%s Ok:%c\n", EVENTs_size,
                 cpu, event_attr.type, event_attr.config, info->name,
                 IS_ERR(evt) ? 'N' : 'Y');
...
void perf_counters_enable(void) {
  ...
  for (i = 0; i < EVENTs_size; ++i) {
    ...
    EVENTs_value[i] = perf_event_read_value(EVENTs[i], &enabled, &running);
