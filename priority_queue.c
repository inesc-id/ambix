...
inline void enqueue_address(priority_queue *pds, unsigned long address, size_t pid, enum access_freq_t access_freq)
{
    if (pds->index[access_freq] < MAX_ADDRESSES) {
        pds->addresses[access_freq][pds->index[access_freq]].addr = address;
        pds->addresses[access_freq][pds->index[access_freq]++].pid_idx = pid;
    }
}
